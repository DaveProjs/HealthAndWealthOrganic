...
trait Global_Widget_Functions {
	...
	protected function render_post_image( $add = [] ) {
		...
		if ( ! $image_src ) {
			printf( '<img class="%1$s" src="%2$s" alt="%3$s">', $wrapper_class, $placeholder_image_src, esc_html( get_the_title() ) );
		} else {
	...
	protected function render_post_title_attr( $id, $add = [] ) {
		...
		printf(
			'<%1$s ' . $this->get_render_attribute_string( $id ) . '><a href="%2$s" title="%3$s">%3$s</a></%1$s>',
			$settings['title_tag'],
			esc_url( get_permalink() ),
			esc_html( get_the_title() )
		);
	...
	protected function render_post_category( $add = [] ) {
		...
		printf(
			...
			$wrapper_class,
			sky_get_post_category( $this->get_settings( 'posts_source' ) )
		);
	}
	...
	protected function render_post_category_attr( $id, $add = [] ) {
		...
		printf(
			'<div ' . $this->get_render_attribute_string( $id ) . '>%1$s</div>',
			sky_get_post_category( $this->get_settings( 'posts_source' ) )
		);
	}
	...
	protected function render_post_author( $add = [] ) {
		...
		// $author_icon = '<i class="sa-post-icon-user"></i>';
		...
		?>
				<div class="<?php print( esc_attr( $wrapper_class ) ); ?>">
					<?php if ( empty( $video_url ) || 'yes' != $settings['show_video'] ) : ?>
							...
							<a class="sa-w-100 sa-h-100" href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo esc_html( get_the_title() ); ?>">
								<?php
